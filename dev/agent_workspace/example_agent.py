"""A worked agent, for reference. Not deployed — copy it into `my_agent.py`.

Three things define an agent, and all three are ordinary Python:
...
`AGENT.md` file, is what `job-inspector` uses; see BACKGROUND_AGENTS.md in the
dltHub AI harness.
"""
...
@agent(
    ...
    limits={"max_turns": 30},
    expose={"display_name": "Example agent"},
)
def example_agent(
